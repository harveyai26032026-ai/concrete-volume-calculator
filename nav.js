...
(function(){
  ...
  function buildDrawer(){
    ...
    SITEMAP.forEach(function(item){
      ...
      if(item.children)item.children.forEach(function(ch){html+='<li><a href="'+ch.href+'" class="dn-tier2">'+ch.label+'</a></li>';});
    });
    html+='</ul>';drawer.innerHTML=html;document.body.appendChild(drawer);
    overlay.addEventListener('click',close);drawer.querySelector('.drawer-close').addEventListener('click',close);
  ...
  var btn=document.querySelector('.hamburger');if(btn)btn.addEventListener('click',function(){if(d&&d.drawer.classList.contains('open'))close();else open();});
